...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  const isBriefPage = project.pageMode === "brief";
  const projectTimeline =
    project.duration === "Independent"
  ...
  return (
    ...
    >
      ...
      {isBriefPage ? (
        <section className="mt-16 grid gap-8 border-t border-[var(--line)] pt-10 md:mt-24 lg:grid-cols-[240px_minmax(0,1fr)]">
          <p className="text-[12px] uppercase tracking-[0.14em] text-[var(--muted)]">
            Notes
          </p>
          <div className="grid gap-6">
            {(project.briefSections ?? []).map((section) => (
              <div
                key={section.heading}
              ...
              >
                <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
                  <div className="space-y-4">
                    ...
                    <p className="max-w-3xl text-[clamp(1.6rem,4vw,3rem)] leading-[1.02] tracking-[-0.05em] text-[var(--ink)]">
                      {project.title}
                    </p>
              ...
              <div className="space-y-5">
                ...
                <div className="grid gap-4">
                  {project.outcomes.map((outcome) => (
                    <div
                      key={outcome}
                      className="info-card text-[14px] leading-6 text-[var(--ink)]"
                    >
                      {outcome}
                    </div>
                  ))}
